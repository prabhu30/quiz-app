...
export default function Quiz({ onSelectAnswer, onSubmit }) {
  ...
  function checkAnswer(optionIndex, selectedAnswer) {
    ...
    if (selectedAnswer === answer) {
      ...
      onSelectAnswer((prevCount) => prevCount + 1);
    }
  }

  return (
    <div className="w-3/5 bg-white p-6 text-left rounded-md mx-auto mt-6">
      <h1 className="text-2xl pb-3 mb-3 border-gray-600 border-b-2">
        Quiz App
      </h1>
      ...
      {options.map((option, index) => (
        <li key={option} className="list-none">
          <button
            className="w-full text-left border border-black p-3 rounded-lg my-2 hover:bg-blue-200 hover:border-blue-600 hover:text-black focus:bg-blue-200 focus:border-blue-600"
